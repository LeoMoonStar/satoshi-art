...
export const getUserCollectibles = (Id: string, pageSize = 8, page = 1) => {
  return axios.get(
    `${process.env.REACT_APP_API}/api/public/user/${Id}/collectibles?pageSize=${pageSize}&page=${page}&tradable=2&user=owner`
  );
};

export const getOnSaleCollectibles = (Id: string, pageSize = 8, page = 1, tradable = 1, user = 'creator') => {
  return axios.get(
    `${process.env.REACT_APP_API}/api/public/user/${Id}/collectibles?pageSize=${pageSize}&page=${page}&tradable=1&user=creator`
  );
};
...
export const getOnHoldCollectibles = (Id: string, pageSize = 8, page = 1) => {
  return axios.get(
    `${process.env.REACT_APP_API}/api/public/user/${Id}/collectibles?pageSize=${pageSize}&page=${page}&tradable=0&user=owner`
  );
};

export const getCreatedCollectibles = (Id: string, pageSize = 8, page = 1) => {
  return axios.get(
    `${process.env.REACT_APP_API}/api/public/user/${Id}/collectibles?pageSize=${pageSize}&page=${page}&user=creator`
  );
};
...
export const putCollectibleOnSale = async (Id: string, data: any) => {
  return axios.put(`${process.env.REACT_APP_API}/api/auth/${Id}/put-on-sale`, data, {
    headers: {
      id: readCookie('id'),
      token: readCookie('token'),
      metamask_address: readCookie('metamask_address'),
...
export const putOnAuction = (collectibleId: any, data: any) => {
  return axios.post(`${process.env.REACT_APP_API}/api/auth/${collectibleId}/put-on-auction`, data, {
    headers: {
      id: readCookie('id'),
      token: readCookie('token'),
      metamask_address: readCookie('metamask_address'),
...
export const dislikeCollectible = (Id: string) => {
  return axios.delete(`${process.env.REACT_APP_API}/api/auth/user/${Id}/unlike`, {
    headers: {
      id: readCookie('id'),
      token: readCookie('token'),
      metamask_address: readCookie('metamask_address'),
...
export const thumbsdownCollectible = (Id: string) => {
  return axios.delete(`${process.env.REACT_APP_API}/api/auth/user/${Id}/unthumbup`, {
    headers: {
      id: readCookie('id'),
      token: readCookie('token'),
      metamask_address: readCookie('metamask_address'),
...
export const createCollectible = async (data: any) => {
  await axios.post(`${process.env.REACT_APP_API}/api/auth/user/collectibles/create`, data, {
    headers: {
      id: readCookie('id'),
      token: readCookie('token'),
      metamask_address: readCookie('metamask_address'),
