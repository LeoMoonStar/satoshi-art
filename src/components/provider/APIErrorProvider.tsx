...
export default function APIErrorProvider({ children }: { children: React.ReactNode }): JSX.Element {
  const [error, updateError] = useState<ErrorType | null>(null);

  const setError = useCallback((title: string, message?: string) => updateError({ title, message }), []);
