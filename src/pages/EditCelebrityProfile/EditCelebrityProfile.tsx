...
const schema = yup.object().shape({
	background: yup.mixed().when('file', {
        is: (file: FileList) => { return (file && file.hasOwnProperty(0) && VALID_FILE_TYPES.includes(file[0].type)) },
        then: yup.mixed()
        	.required('A background is required')
        	.test('fileRequired', 'Background is required', (value) => value && value.hasOwnProperty(0))
            .test('fileSize', 'The background is too big. You need to upload a smaller one', (value) => value && value.hasOwnProperty(0) && value[0].size <= FILE_SIZE)
    ...
    imageUrl: yup.mixed().when('file', {
        ...
        then: yup.mixed()
            ...
            .test('fileSize', 'The image is too big. You need to upload a smaller one', (value) => value && value.hasOwnProperty(0) && value[0].size <= FILE_SIZE)
            .test('fileFormat', 'Unsupported Format', (value) => value && value.hasOwnProperty(0) && VAlID_IMAGES_TYPES.includes(value[0].type)),
    }),
...
export default function EditCelebrityProfile() {
	...
	const { register, handleSubmit, setValue, control, watch, formState: { errors }} = useForm({ resolver: yupResolver(schema), defaultValues: { 
        theSeries: 'this is the series', theHeader: 'this is the header', year: '', month: '', date: '',
        introduction: 'this is the introduction', conclusion: 'this is conclusion'
    }})
	const [images, setImages] = useState({ 
		background: '', backgroundBase64: '', backgroundImagetype: '',
		imageUrl: '', imageUrlBase64: '', imageUrlImagetype: ''
	})
	...
	const submit = async (data: any) => {
		...
		const profile = {
			...
			theSeries: theSeries, header: theHeader, date: updatedate, introduction: introduction,
			bulletPoints: bulletPoints, conclusion: conclusion
		}
		...
		<Layout headerVariant="full">
			<form className={classes.form} onSubmit={handleSubmit(submit)}>
				<div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
					<div className={classes.upload}>
	                    ...
	                    <div className={clsx(classes.uploadWrapper)}>
	                        <input accept={ALL_SUPPORTED_TYPES} className={classes.input} ref={register} onChange={handleFileChange} id="uploadBackground" name="background" type="file" hidden/>
	                        ...
	                        {images.background && <IconButton className={classes.closeBtn} onClick={() => {
	                ...
	                <div className={classes.upload}>
	                    ...
	                    <div className={clsx(classes.uploadWrapper)}>
	                        <input accept={ALL_SUPPORTED_TYPES} className={classes.input} ref={register} onChange={handleFileChange} id="uploadImageUrl" name="imageUrl" type="file" hidden/>
	                        ...
	                        {images.imageUrl && <IconButton className={classes.closeBtn} onClick={() => {
	            ...
	            <FormControl>
		            ...
		            {errors.year && <small style={{ marginTop: 10 }} className={classes.inputError}>{errors.year.message}</small>}
		            {errors.month && <small style={{ marginTop: 30 }} className={classes.inputError}>{errors.month.message}</small>}
		            {errors.date && <small style={{ marginTop: 50 }} className={classes.inputError}>{errors.date.message}</small>}
		        </FormControl>

	            <FormControl className={classes.fieldGroup}>
	                <label htmlFor="introduction">Introduction</label>
	                <TextField multiline id="introduction" name="introduction" inputRef={register} className={classes.textField}/>
	            ...
	            <FormControl>
		            <div className={classes.bulletpoints}>
		            	...
		            	<div className={classes.bulletpointsAdd} onClick={addPoints}>Add a point</div>
		            	<div style={{ display: 'flex', flexDirection: 'column' }}>
			            	{bulletPoints.map((point, index) => (
			            		<div style={{ display: 'flex', flexDirection: 'row', margin: '3px 0' }} key={index}>
			            			<input className={classes.bulletpointinput} placeholder={"#" + (index + 1) + ": point"} onChange={(e) => editPoint(index, e.target.value)} value={point}/>
			            			<div className={classes.bulletpointdelete} onClick={() => deletePoint(index)}>Delete #{index + 1} point</div>
			            		</div>
			            	))}
	            ...
	            {isErrors() && <p className={classes.textError}>There were some issues. Please see above what you need to fix and try again. The button will be enabled after you apply your changes.</p>}

	            <div className={classes.footer}><Button type="submit">Update</Button></div>
	        </form>
		</Layout>
	)
}