...
import { PriceIcon, TransferIcon } from '../../../../components/icons/dashboard';
    ...
    <div className={classes.container}>
      <div className={classes.head}>
        <div className={classes.intro}>
          <h3 className={classes.mainTitle}>{text['pieChart']}</h3>
        </div>
        <div className={classes.checkboxGroup}>
          ...
          <IconButton className={classes.showMoreButton} buttonRef={anchorElRef} onClick={() => setOpen(!isOpen)}>
      ...
      {items.map((data, index) => (
        <div className={classes.col} key={index}>
          <PieCard color={data.color} value={data.value} subTitle={data.subTitle} />
        </div>
      ))}
    </div>
  );
