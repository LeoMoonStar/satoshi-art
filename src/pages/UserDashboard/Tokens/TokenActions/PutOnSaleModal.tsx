...
import { convertEthToUsd, convertStringToNumber, etherToWei } from 'utils/helpers';
import { putTokenOnSaleAPI, Token } from 'apis/token';
...
const schema = yup.object().shape({
  price: yup.number().required('You need to enter the price').typeError('"Price" must be a number'),
  copiesCount: yup.number().typeError('You need to enter number'),
});

export default function PutOnSaleModal({ onClose, token, onSuccess }: PutOnSaleModalProps): JSX.Element {
  ...
  const [isPutOnSaleInProgress, setPutOnSaleInProgress] = useState<boolean>(false);
  ...
  const isNeedToApprove = token.metadata.type === TokenType.MULTIPLE && !isApproved;
  ...
  const putTokenOnSaleBlockchain = async (data: PutOnSaleForm) => {
    ...
    if (isSingle) {
      const putSingleTokenOnSaleResponse = await singleContract.putOnSale(tokenId, priceInWei, { from: account });
      return {
        price: priceInWei,
  ...
  const setApprovalForAllBlockchain = async () => {
    ...
    try {
      await erc1155contract.setApprovalForAll(engine1155contract.address, true, {
        from: account,
      });
      setActiveStep(1);
    } catch (e) {
  ...
  const checkIsApprovedForAllBlockchain = useCallback(async () => {
    if (engine1155contract) {
      const isApproved = await erc1155contract.isApprovedForAll(account, engine1155contract.address);
  ...
  const handleNumberInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    setValue(e.target.name as keyof PutOnSaleForm, e.target.value.replace(/\D/g, ''));
  };
      ...
      <form className={classes.container} onSubmit={handleSubmit(onFormSubmit)}>
        ...
        <FormControl className={classes.fieldGroup}>
          ...
          <Input
            ...
            id='price'
            placeholder='1'
            endAdornment={<div className={classes.priceValueType}>ETH</div>}
          />
          {errors.price && <p className={classes.textError}>{errors.price.message}</p>}
        </FormControl>
        {token.metadata.type === TokenType.MULTIPLE && token.metadata.payload.copiesCount && (
          <FormControl className={classes.fieldGroup}>
            <InputLabel shrink htmlFor='quantity'>
              {text['enterQuantity']}{' '}
              <small>
                {
                  text['countAvailable'] //, {count: token.metadata.payload.copiesCount,})
                }
              </small>
            </InputLabel>
            {/*@TODO: add validation that not allow user to put more tokens than he has*/}
            <Input inputRef={register} name='copiesCount' onChange={handleNumberInput} id='quantity' placeholder='1' />
            {errors.copiesCount && <p className={classes.textError}>{errors.copiesCount.message}</p>}
          </FormControl>
        )}
        <ul className={classes.additionalInfo}>
          <li>{/* <Trans i18nKey='putOnSaleServiceFee' values={{ fee: '2.5' }} components={{ b: <b /> }} /> */}</li>
          <li>
        ...
        <div className={classes.buttons}>
          <Button type='submit' variantCustom='action' className={classes.buttonFilled}>
            {text['nextStep']}
          </Button>
          <Button onClick={onClose} variantCustom='outlined' className={classes.cancelButton}>
            {text['cancel']}
          </Button>
        </div>
      </form>
