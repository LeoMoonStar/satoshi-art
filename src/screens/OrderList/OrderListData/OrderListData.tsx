        ...
        return [
            {
                ...
                renderCell({ row }: { row: RowType }) {
                    return (
                        <Link to={`dashboard/order-list/${row.id}`}>
                            #{row.id}
                        </Link>
                    )
                },
            ...
            { field: 'artist', headerName: t('artist'), flex: 1 },
            {
                field: 'artName',
                headerName: t('artName'),
                flex: 2,
                renderCell({ row }: { row: RowType }) {
                    return (
                        <Link to={`dashboard/order-list/${row.id}`}>
    ...
    return (
        <>
            <DataGrid
                ...
                rows={rows}
                components={{
                    ColumnSortedDescendingIcon: SortedArrowsUpIcon,
                    ColumnSortedAscendingIcon: SortedArrowsDownIcon,
                }}
                columns={columns}
                pageSize={10}
                autoHeight
