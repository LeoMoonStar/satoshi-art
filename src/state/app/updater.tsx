...
import { updateBlockNumber, changeWhitelistedStatus, changeRateEthToUsd } from './actions';
...
export const useUserWhiteListChecking = (): void => {
  ...
  useEffect(() => {
    ...
    // checkUserWhitelisted(account).then((res) => dispatch(changeWhitelistedStatus(res.isWhitelisted)))
  }, [dispatch, account]);
};
...
export function useUpdateBlockNumber(): void {
  ...
  const blockNumberUpdateEventHandler = useCallback(
    (blockNumber: number) => dispatch(updateBlockNumber(blockNumber)),
    [dispatch]
  );
