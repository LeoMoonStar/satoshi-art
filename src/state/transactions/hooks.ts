...
export function useTransactions(): TransactionsListItem[] {
  ...
  const transactions = useSelector<AppState, Record<string, TransactionsListItem> | undefined>((state: AppState) =>
    chainId ? state.transactions.transactions[chainId] : undefined
  );
...
export function usePendingTransactions(): PendingTransaction[] {
  ...
  return useMemo(() => {
    return transactions.filter((tx): tx is PendingTransaction => !tx.receipt); // pending transactions don't have receipt
  }, [transactions]);
}
