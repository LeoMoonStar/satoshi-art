...
export function shortAddress(address: string, slashIndex = 10): string {
  ...
  return address.slice(0, slashIndex) + '...' + address.slice(addressLength - 4, addressLength);
}
...
export const getKeyValue =
  <T extends Record<string, unknown>, U extends keyof T>(
    obj: T
...
export function convertEthToUsd(value: number, currency: number, fee = 0.025, format = '0,0.00'): string {
